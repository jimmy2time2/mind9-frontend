...
import { getTwitterClient, sendTweet, isUsingMockClient } from './twitterClient';
...
export class SocialManager {
  ...
  private async initializeTwitter() {
    try {
      ...
      // Check if Twitter client can be initialized
      const client = getTwitterClient();
      
      if (!client) {
        this.lastError = new Error('Failed to initialize Twitter client');
        return;
      }
